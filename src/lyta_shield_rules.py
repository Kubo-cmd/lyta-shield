...
BLOCKED_PATTERNS: List[Tuple[str, str]] = [
    (r"(echo\s+['\"]?[A-Za-z0-9+/=\s]{40,}['\"]?\s*\|\s*base64\s+-d\s*\|\s*(ba)?sh)\b", "base64_decode_to_shell"),
    (r"(base64\s+-d\s*.*?\|\s*(ba)?sh)\b", "base64_decode_to_shell"),
    ...
    (r"(?:copy|paste|run|execute|type|put|enter)\s+(?:this\s+)?(?:code|command|script|text|thing)?\s+(?:in|into|inside|to)\s+(?:the\s+)?(?:terminal|shell|console|prompt|window|box|field)\b", "paste_jacking_instruction"),
    (r"\b(rm\s+-rf\s+/|rm\s+-rf\s+~|mkfs\.|>:\s*\{\}\;|fork\s+bomb)\b", "destructive_command"),
    (r"\b(sudo\s+)?(chmod\s+\+x\s+/tmp/.*\s*\&\&\s*\S+/tmp/\w+)\b", "download_and_execute_tmp"),
    (r"\b(nc\s+-e\s+|netcat\s+-e\s+|bash\s+-i\s+>&\s+/dev/tcp/)\b", "reverse_shell"),
...
SUSPICIOUS_PATTERNS: List[Tuple[str, str]] = [
    ...
    (r"\b(echo\s+['\"]?[A-Za-z0-9+/=\s]{20,}['\"]?\s*\|\s*base64)\b", "base64_decode_without_shell"),
    (r"\b(?:openssl|gpg)\s+enc\b.*?\|\s*(ba)?sh\b", "encrypted_payload_to_shell"),
    (r"\b(eval\s*\(|\beval\s+['\"]\$|\bexec\s+\$\(curl)\b", "eval_dynamic_exec"),
    (r"\b(paste\s+(?:this\s+)?(?:code|command|text))\b", "paste_instruction_weak"),
    (r"\b(copy\s+(?:the\s+following\s+)?(?:code|command|script))\b", "copy_instruction_weak"),
    ...
    (r"\b(Worker\s*\(|SharedWorker\s*\(|importScripts\s*\(\s*['\"]https?://)", "browser_worker_remote"),
]
