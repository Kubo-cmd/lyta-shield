...
3. ~/.config/lyta-shield/github-token (not recommended; use keychain)
...
# Fallback: local file (for CI/non-macOS; warn)
...
print("  3. gh CLI: gh auth login (token will be read automatically)", file=sys.stderr)
print("  security add-generic-password -s 'lyta-shield-github-token' -a 'lyta-shield' -w '<TOKEN>'", file=sys.stderr)
print("  For 1Password: export OP_GITHUB_TOKEN=$(op read 'op://Private/lyta-shield-github-token/token')", file=sys.stderr)
sys.exit(1)
